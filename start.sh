...
if [ $i -eq 15 ] && ! nc -z 127.0.0.1 50051 2>/dev/null; then
...
if [ ! -f "$VENV_DIR/bin/python" ]; then
    ...
    "$VENV_DIR/bin/pip" install -q services/gateway/ services/llm-orchestrator/
fi
...
cd "$SCRIPT_DIR"
PYTHONPATH=services/llm-orchestrator:services/gateway \
    "$VENV_DIR/bin/python" -m uvicorn atrium.app:app --host 0.0.0.0 --port 8080 &
GW_PID=$!

echo ""
echo "========================================"
echo "  Atrium 已启动!"
echo "  Rust 后端:  PID=$RUST_PID  (gRPC 127.0.0.1:50051)"
echo "  Python 网关: PID=$GW_PID (HTTP 127.0.0.1:8080)"
echo "  Web 控制台:  http://localhost:8080"
echo "  数据目录:    $ATRIUM_HOME"
echo "  停止:        kill $RUST_PID $GW_PID"
