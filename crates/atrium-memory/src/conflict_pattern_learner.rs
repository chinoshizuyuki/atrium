...
fn stage_to_idx(stage: &RelationshipStage) -> usize {
    match stage {
...
mod tests {
    use super::*;
    use crate::conflict_reconciliation::ConflictIntensity;
    ...
    fn test_pattern_absorb() {
        ...
        assert_eq!(p.stage_distribution[0], 1); // Acquaintance
    ...
    fn test_pattern_absorb_multiple_stages() {
        ...
        assert_eq!(p.stage_distribution[0], 1); // Acquaintance: 1
        assert_eq!(p.stage_distribution[3], 2); // Deep: 2
    }
    ...
    fn test_pattern_decay() {
        ...
        // After 100 decays at 0.995: 0.995^100 ≈ 0.606
    ...
    fn test_pattern_should_prune() {
        ...
        p.frequency = 2;
        p.decay_weight = 0.05;
        assert!(p.should_prune(0.1)); // weight < 0.1 && freq < 3

        p.frequency = 5;
        assert!(!p.should_prune(0.1)); // freq >= 3, not pruned

        p.frequency = 2;
        p.decay_weight = 0.5;
        assert!(!p.should_prune(0.1)); // weight >= 0.1
    }
    ...
    fn test_learner_predict_with_match() {
        let mut learner = ConflictPatternLearner::default();
        // Learn multiple times with short trigger keyword that is a substring of prediction text
        for i in 0..5 {
    ...
    fn test_learner_predict_stage_sensitivity() {
        ...
        // Deep stage should have higher sensitivity than acquaintance
    ...
    fn test_learner_suggest_sensitivity() {
        ...
        // High frequency (>10) + acquaintance (0.6) → multiplier < 1.0
        let adj = adjustments
            .iter()
    ...
    fn test_learner_tick_decay_and_prune() {
        ...
        // Heavy decay to push below prune threshold
        for i in 0..500 {
            learner.tick(2000 + i);
        }
        // Pattern with freq=1 and heavy decay should be pruned
    ...
    fn test_learner_tick_preserves_high_freq() {
        let mut learner = ConflictPatternLearner::default();
        // Learn 5 times to build frequency
        for i in 0..5 {
            let sig = make_signal(ConflictType::ValueConflict, "不同意", 1000 + i * 100);
            learner.learn(&[sig], &deep_stage(), 1000 + i * 100);
        }
        // Even after many decays, freq >= 3 should not be pruned
        for i in 0..200 {
            learner.tick(5000 + i);
        }
        assert!(!learner.patterns.is_empty());
    ...
    fn test_pattern_keyword_dedup() {
        ...
        let sig2 = make_signal(ConflictType::OverDemand, "帮我", 2000);
        p.absorb(&sig2, &deep_stage(), 2000);
        // Same keyword should be deduped
